...
description: 稼働実績（縦一列カレンダー形式）の PDF を生成する。対象年月と日別の稼働明細（時間帯・作業内容）を会話や引数から受け取り、固定デザインのテンプレートに流し込んで Chrome ヘッドレスで PDF 化する。ユーザーが「稼働実績作って」「稼働表作って」「作業実績まとめて」と言ったとき、または /freelance-work-record を実行したときに使用。
---
