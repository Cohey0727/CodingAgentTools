...
### 8. 統合 (fusion) と総括 — ホストLLMが行う

全レビューの回収後、**ホストLLM (このセッションの Claude、オーケストレーター)** が各モデルの指摘を突き合わせて統合し、最後に総括する。レビュアーの1人としての `schema: self` の役割とは別で、ここでは裁定者として振る舞う:
...
## 単独指摘 (1モデルのみ — 要検証)
- [重要度] 内容 (指摘: mmx) — ホストが根拠をコード上で検証した結果

## モデル間で矛盾した点
- 論点 / 各モデルの立場 / ホストの裁定と根拠

## 総括 (ホストLLM)
- 変更全体の評価を数行で: 品質・リスクの所感、このままマージ可能か / 修正必須か
- 対応すべき指摘の優先順リスト (対応不要と裁定した指摘は理由を明記)
- 各レビュアーの回答品質への短評 (どのモデルの指摘が有効だったか)
```

単独指摘・矛盾点は必ずホストがコードや事実に当たって裁定する。多数決を鵜呑みにしない (全員が同じ誤解をすることもある)。総括は必ずホストLLM自身の言葉で書き、各モデルの出力の切り貼りで済ませない。
